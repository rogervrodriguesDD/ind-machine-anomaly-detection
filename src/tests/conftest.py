...
from ind_machine_anomaly_detection.data.machine_data import (
        MachineSensorDataCSV
        )
...
def test_labels_data_catalog_with_missing_labels_encoding(test_config):
    ...
    labels_encoding_dict = labels_data_catalog.machine_data_labels_encoding

    labels_encoding_dict_with_missing_labels = {
        k: int(labels_encoding_dict[k])
        for k in list(labels_encoding_dict.keys())[:-1]
    }

    labels_data_catalog_with_missing_labels_encoding = LabelDataCatalog(
        machine_data_labels_file=labels_data_catalog.machine_data_labels_file,
...
def test_machine_data(test_config):
    test_machine_data = MachineSensorDataCSV(
...
def test_machine_data_with_missing_values(test_machine_data):
    ...
    samples_list = idx_original_df.get_level_values(level=0).unique().tolist()

    # Instance data
    samples_insert_nan = set(
        np.random.choice(samples_list, size=10, replace=False)
        )
    ...
    # Labels data
    samples_insert_nan_labels = np.random.choice(
        samples_list,
        size=10,
        replace=False
        )
