...
def test_class_spec_freq_scaler():
    ...
    # Creating example data: a set of sine signals (scaled to range 0.0
    # and 1.0), exponentiated (given that the scaler calculate the logarithm)
    N_samples = 4
    N_points = 2000
    ...
    for i in range(N_samples):
        f = np.random.randint(low=1, high=10, size=1)
    ...
    df = pd.DataFrame(
        X_exp,
        columns=[f"ch1_X_{i}" for i in range(1, N_points + 1)]
        )

    # Scaling the data
    df_scaled = SpecFreqScaler(channel='ch1').fit_transform(df)

    assert np.allclose(X, df_scaled.values, atol=1e-4)
...
def test_class_freq_pos_min_max_scaler():
    ...
    df = pd.DataFrame(
        X,
        columns=[f"ch1_freqmax_{i}" for i in range(1, N_points + 1)]
        )

    # Counting number of min and max values
    # (must be equal after transformation)
    max_X = X.max()
    min_X = X.min()
...
def test_class_min_max_scaler_customized():
    ...
    cols_names = [
        f"ch1_{col}_{i}"
        for col in variables_with_mult_cols
        for i in range(1, N_cols_mult + 1)
        ]
    ...
    cols_names.extend(
        [
            ...
            if col not in variables_with_mult_cols
        ])

    df = pd.DataFrame(X, columns=cols_names)

    # Scaling the data
    df_scaled = MinMaxScalerCustomized(
        channel='ch1',
    ...
    with pytest.raises(OSError) as excinfo:
        MinMaxScalerCustomized(
            channel='ch1',
            variables=variables_names + ['mean']
            ).fit_transform(df)

    expected_error = \
        "Variable 'mean' related to channel ch1 not found in input data"
