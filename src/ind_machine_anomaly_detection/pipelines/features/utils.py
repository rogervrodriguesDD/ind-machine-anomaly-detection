...
def calc_fft(df, col, samples, N, fs):
    """
    ...
    Args:
        df (pd.DataFrame): Multi-level index DataFrame with signal data.
                            Expected indices order: Sample identification, measurement point
        col (str): Name of the column to be used when calculating the FFT
        samples (list): List of samples indices to be used for filtering the DataFrame
        N (int): Number of measurement points of the original signal
        fs (float): Sampling frequency

    Returns:
        f (np.array): Vector with the N/2 frequency values  for plotting the Spectrum.
        mat_X (np.array): Array with shape (N/2, number of samples) containing the absolute value
                            of the Discrete Fourier Tranformation of the Signal. The limit frequency
                            is considered to be half of the sampling frequency (therefore N/2).
    """
...
def create_df_esp_freq(X_mat, samples, N_max, col, axis_concat=0, data_freq = None):
    """
    Create a DataFrame for a given Spectral output of a FFT operation and its informations.
    If a previous created DF is passed as `data_freq` argument, the new DF will be concated
    to it using the `axis_concat` argument as reference.

    Args:
        X_mat (np.array): Matrix containing the output of the FFT operation (Spectrum in frequency).
        samples (list): List of samples indices to be used when creating the Multi-level index.
    ...
    Returns:
        data_freq (pd.DataFrame): When None, a new DataFrame is returned. Else, the new DF is concatenated
                                to the given one, considering the `axis_concat` reference.
    """
    ...
    data_freq_ = pd.DataFrame(data=X_mat[:N_max],
                            columns=samples,
                            index=indexes_)
...
def calculate_spec_frequency_all_instances(data, N_max):
    """
    Apply the FFT on all samples measurements, for the three channels, and returns
    a concatenated DataFrame with the results.

    Args:
        data (MachineSensorDataCSV): Object that contains all datasets of the machine sensors.
        N_max (int): Maximum number of frequency points to be returned as columns (for each channel).

    Returns:
        data_spec_freq (pd.DataFrame): DataFrame with the spectral output, where the rows are related to
                                        the sample identification and columns are the amplitude of the
                                        i-th measured frequency.
    """
    ...
    for col in ['ch1', 'ch2', 'ch3']:
        ...
        _, fft = calc_fft(df_samples,
                        col,
                        idx_samples,
                        N=data.instances_data_catalog.machine_data_instances_number_meas_points,
                        fs=data.instances_data_catalog.machine_data_instances_sampling_frequency)

        # Creating a dataframe with the amplitudes in the frequency domain
        axis_concat = 1
        data_spec_freq = create_df_esp_freq(fft, idx_samples, N_max, col, axis_concat, data_spec_freq)

    return data_spec_freq

def get_freq_peaks_positions_values(data_f, channel, min_perc_samples_count=0.25):
    """
    ...
    Args:
        data_f (pd.DataFrame): DataFrame with Spectral output of FFT on the data
        channel (str): Name of the channel (first level column)
        min_perc_samples_count (int): Percentual of samples considered to return the peak

    Returns:
        data_freq_peaks (pd.DataFrame): DataFrame with peaks positions (columns names) and
    ...
    count_peaks = pd.DataFrame([0] * len(data_f[channel].columns),
    ...
    get_peaks_idx = count_peaks.loc[count_peaks['count'] > min_perc_samples_count * len(data_f[channel])].index
...
def get_freq_peaks_positions_values_all_channels(data_freq, min_perc_samples_count=0.25):
    """
    ...
    Args:
        data_freq (pd.DataFrame): DataFrame with Spectral output of FFT on the data
    ...
    for i, channel in enumerate(list_channels):
        data_freq_peaks_ = get_freq_peaks_positions_values(data_freq,
                                                        channel=channel,
...
def get_ordered_peaks_positions(df, channel, n_freq_points, n_max_positions=3):
    """
    ...
    Args:
        ...
        channel (str): Name of the channel
        n_freq_points (int): Total number of frequency points of the original spectrum of frequency
        n_max_positions (int): Number of ordered peaks desired

    Returns:
        data_ord_peak_pos (pd.DataFrame): DataFrame with ordered peaks positions.
    """
    ...
    for idx_df, row in df_.iterrows():
        ...
        for pos in range(1, n_max_positions+1):
            ...
            array_peaks_pos[i-1, pos-1] = position / n_freq_points

        i += 1

    data_ord_peak_pos = pd.DataFrame(data=array_peaks_pos,
                                 columns = ['{}_freqmax_{}'.format(channel, i) for i in range(1, n_max_positions+1)],
                                 index = df_.index)

    return data_ord_peak_pos

def get_ordered_peaks_positions_all_channels(data_freq_peaks, n_freq_points, n_max_positions=5):
    """
    Get the normalized peaks positions ordered by the values peaks for all channels,
    using the function `get_ordered_peaks_positions`.
    ...
    list_channels = ['ch1', 'ch2', 'ch3']
    for i, channel in enumerate(list_channels):
        data_ord_peaks_pos_ = get_ordered_peaks_positions(data_freq_peaks,
                                                        channel=channel,
                                                        n_freq_points=n_freq_points,
                                                        n_max_positions=n_max_positions)
        list_data_peaks_pos.append(data_ord_peaks_pos_)

    data_ord_peaks_pos = pd.concat(list_data_peaks_pos, axis=1, ignore_index=False)
...
def calculate_thdn(df, col, samples, N=70000, beta_filter=38, n_harm=5, wind_fund_ratio=0.1, f_max=250):
    """
    Calculate the Total Harmonic Distortion plus Noise for a the given set of signals in time domain.
    ...
    Args:
        ...
        beta_filter (float): Shape parameter of the Kaiser window function (Default = 38)
        n_harm (int): Number of harmonic regions, in addition to fundamental, considered
                    in the spectrum frequency (Default = 5)
        wind_fund_ratio (float): Size of the window considered in the harmonic regions
                                (Default = 0.1)
        f_max (int): Maximum frequency considered (Default = 250)

    Returns:
        samples_sorted (list of ints): List of sorted samples id
        f_fund (list of floats): List with fundamental frequencies founded per sample
...
def get_df_thdn_by_channel(data, pipeline_config, channel='ch1', add_channel_prefix=True):
    """
    ...
    Args:
        data (pd.DataFrame): DataFrame with set of signals in time domain.
        pipeline_config (PipelineConfig): Configuration object with needed parameters to
                                        call function `calculate_thdn`.
        channel (str): Name of the channel.
        add_channel_prefix (bool): If True, add the channel name to columns as prefix.

    Returns:
        df_thdn (pd.DataFrame): DataFrame with results of the TDHn estimation
    """

    samples = data.dataframe_instances.index.get_level_values(level=0).unique()
    ...
    df_thdn = pd.DataFrame(index=samples_sorted,
                           data={'ffund': f_fund,
                                 'rms_total' : rms_total,
                                'rms_noise': rms_noise})
    ...
    if add_channel_prefix:
        rename_cols = {col : f"{channel}_{col}" for col in df_thdn.columns}
...
def calculate_thdn_all_channels(data, pipeline_config):
    """
    ...
    Args:
        data (pd.DataFrame): DataFrame with set of signals in time domain
        pipeline_config (PipelineConfig): Configuration object with parameters needed
                                        to estimate tdhn (see `calculate_thdn` documentation).
    ...
    for channel in list_channels:
        df_thdn_ = get_df_thdn_by_channel(data, pipeline_config, channel=channel)
        list_df_thdn.append(df_thdn_)

    df_thdn = pd.concat(list_df_thdn, axis=1, ignore_index=False)
    return df_thdn

def calculating_stats_summary_all_instances(data):
    """
    Calculate a statistical summary table on all instances data (samples and channels)
    in time domain. The parameters calculated are: mean, median, kurtosis, and skewness.

    Args:
        data (pd.DataFrame): DataFrame with signal data in time domain

    Returns:
        df_stats_summary (pd.DataFrame): DataFrame with statistical summary table
    """
    list_dfs = []
    df_, _ = data[:]
    for col in ['ch1', 'ch2', 'ch3']:
        df_stats_summary_ = df_[col].unstack().agg(['mean', 'median', 'kurtosis', 'skew'], axis=1)
